...
public class SortingWithComparator {
	...
	public static void main(String[] args) {
		...
		// sorting based on salary
				...
				.map(e -> e.getName() + " gets " + e.getSalary())
				.collect(Collectors.toList())
				.forEach(System.out::println);
		System.out.println("\n\n\n");
		...
		List<String> result = employeeStream
								.filter(o-> o!= null)
								.filter(e -> (e.getSalary() > 90000 && e.getAge() > 30 && e.getAge() < 40))
								.sorted(Comparator.comparing(Employee::getSalary))
								.map(e -> e.getName() + " is just "  + e.getAge() + " years old but gets " + e.getSalary())
								.collect(Collectors.toList());
		...
		employeeStream = Arrays.stream(EmployeeGenerator.getEmployeeArray());
		Employee matchingEmployee = employeeStream
									.filter(o-> o!= null)
									.filter(e -> (e.getSalary() > 90000 && e.getAge() > 30 && e.getAge() < 40))
									.findAny().orElse(null);
