...
class TransactionTest {
	...
	public static void main(String[] args) {
		...
		try{
			...
			Converter<Transaction, Transaction> convertToUSD = currencyConverter::getUSD;
			
			System.out.println("\n sorting by USD value");
			transactionList.stream().map(convertToUSD::convert).sorted().forEach(System.out::println);
			
			System.out.println("\n sorting by currency name"); 
			transactionList.stream().map(convertToUSD::convert).sorted(Comparator.comparing(Transaction::getCurrency)).forEach(System.out::println);
